...
updateEclipseClasspathFile = { newPlugin = null ->
    ...
    def handlePluginDirectory = grailsSettings.pluginDependencyHandler()
    ...
    config.grails.plugin.location.each{base ->
        ...
        handlePluginDirectory(new File(base.value))
    }
    ...
    String userHomeRegex = isWindows ? userHome.toString().replace('\\', '/') : userHome.toString()
    String grailsHomeRegex = isWindows ? grailsHome.toString().replace('\\', '/') : grailsHome.toString()
    String baseRegex = isWindows ? basedir.toString().replace('\\', '/') : basedir.toString()
        ...
        def normalizeFilePath = { file ->
            ...
            if(isWindows){
              path = path.replace('\\', '/')
						...
						//println "[normalizeFilePath]path=$path"
            String originalPath = path
            path = path.replaceFirst(~/$grailsHomeRegex/, 'GRAILS_HOME')
            path = path.replaceFirst(~/$userHomeRegex/, 'USER_HOME')
			      //path = path.replaceFirst(~/$baseRegex/, '')
			      //path = path.replaceFirst(~/$baseRegex/, 'PROJECT_LOC')
...
updateEclipseProjectFile = { newPlugin = null ->
    ...
    xml.projectDescription {
      ...
      //linked plugin 2011/07/27 kimukou.buzz add start
      def config  = new ConfigSlurper().parse(new File('grails-app/conf/BuildConfig.groovy').toURL())
      config.grails.plugin.location.each{
